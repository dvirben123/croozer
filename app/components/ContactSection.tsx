...
export function ContactSection({ onOpenTermsDialog }: ContactSectionProps) {
  ...
  return (
    <section ref={elementRef} className="py-16 sm:py-20 px-4 sm:px-6 bg-accent/30">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <h2 className="text-responsive-h2 mb-4 sm:mb-6 text-foreground">
            מוכנים להתחיל?
            <span className="text-primary block !text-[24px] sm:!text-[32px]">בואו נדבר</span>
          </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6" dir="rtl">
              {[
                { id: "fullName", label: "שם מלא *", type: "text", placeholder: "השם המלא שלכם", dir: "rtl" },
                { id: "businessName", label: "שם העסק *", type: "text", placeholder: "שם בית העסק שלכם", dir: "rtl" },
                { id: "phone", label: "מספר טלפון *", type: "tel", placeholder: "050-1234567", dir: "rtl" },
                { id: "email", label: "כתובת מייל *", type: "email", placeholder: "email@example.com", dir: "ltr" }
              ].map((field, index) => (
                <motion.div
                  key={field.id}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  <label htmlFor={field.id} className="block text-muted-foreground mb-2 text-right text-responsive-base">{field.label}</label>
                  <Input
                    id={field.id}
                    name={field.id}
                    type={field.type}
              ...
              >
                <label htmlFor="message" className="block text-muted-foreground mb-2 text-right text-responsive-base">ספרו לנו על העסק שלכם</label>
                <Textarea
                  id="message"
                  name="message"
                  rows={4}
            ...
            <div className="bg-gradient-to-r from-primary/20 to-secondary/20 rounded-lg p-6 sm:p-8 text-center border border-border h-full flex flex-col justify-between">
              <div>
                ...
                <div className="mb-6 sm:mb-8 space-y-3 sm:space-y-4">
                  {[
                    { icon: "⚡", title: "מענה מיידי", desc: "אנחנו זמינים לעזור לכם" },
                    { icon: "🎯", title: "יעוץ אישי", desc: "קבלו המלצות מותאמות לעסק שלכם" },
                    { icon: "💡", title: "הדגמה חיה", desc: "ראו את הבוט פועל בזמן אמת" }
                  ].map((item, index) => (
                    <motion.div 
                      key={index}
                    ...
                    >
                      <p className="text-responsive-base text-card-foreground mb-1 sm:mb-2">{item.icon} {item.title}</p>
        ...
        >
          <div className="text-center">
            ...
            >
              <span className="text-responsive-base text-foreground">בוט וואטסאפ</span>
              <MessageCircle className="h-6 w-6 sm:h-8 sm:w-8 text-primary mr-2" />
            </motion.div>
            <motion.p 
            ...
            >
              © 2024 בוט וואטסאפ. כל הזכויות שמורות.
            </motion.p>
          </div>
        </motion.div>
      </div>
